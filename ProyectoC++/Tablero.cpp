...
void Tablero::imprimir() {
	...
	const std::string AMARILLOCLARO = "\033[93m";     // Amarillo claro
	const std::string MAGENTA = "\033[35m";     // Magenta
	const std::string CIAN = "\033[36m";     // Cian
	...
	while (fila) {
		Nodo* actual = fila;
		while (actual) {
			// Asignar colores según el símbolo
			if (actual->simbolo == '#') {
				std::cout << VERDE << actual->simbolo << RESET << " ";
			}
			else if (actual->simbolo == '@') {
				std::cout << CIAN << actual->simbolo << RESET << " ";
			}
			else if (actual->simbolo == '$') {
				std::cout << MAGENTA << actual->simbolo << RESET << " ";
			}
			else if (actual->simbolo == '!') {
				std::cout << AMARILLOCLARO << actual->simbolo << RESET << " ";
			}
			else {
